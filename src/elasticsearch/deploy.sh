...
# Usage: ./deploy.sh [--forwarding|--devcontainer|--letsencrypt|--step-ca] [--keycloak] [--keycloak-network]
...
build_compose_command() {
    ...
    if [ "$KEYCLOAK_ENABLED" = true ]; then
        ...
        compose_cmd="$compose_cmd --profile keycloak"
    ...
    compose_cmd="$compose_cmd up -d --build"
    echo $compose_cmd
}
...
case "$1" in
    ...
    --step-ca|-sc)
        ...
        ;;
    --help|-h|*)
        echo "Usage: $0 [--forwarding|--devcontainer|--letsencrypt|--step-ca] [--keycloak] [--keycloak-network]"
        echo ""
